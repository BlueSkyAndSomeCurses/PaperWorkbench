...
class CodeAPI:
    def __init__(self, base_url: Optional[str] = None, timeout: float = 60.0, token: Optional[str] = None):
        """
        ...
        """
        if not base_url:
            base_url = os.environ.get("CODEAPI_URL", "https://api.codapi.org/v1/exec")
    ...
    def extract_images_base64(response_json: Dict[str, Any]) -> List[str]:
        ...
        if isinstance(response_json.get("images"), list):
            images.extend([img for img in response_json["images"] if isinstance(img, str)])
        ...
        if isinstance(files, dict):
            for name, content in files.items():
                ...
                if isinstance(content, dict):
                    data = content.get("content") or content.get("base64") or content.get("b64")
                elif isinstance(content, str):
                    data = content
                if isinstance(name, str) and name.lower().endswith((".png", ".jpg", ".jpeg", ".svg")) and isinstance(data, str):
                    images.append(data)
        if not images:
            stdout = response_json.get("stdout")
            if isinstance(stdout, str) and stdout:
                pattern = r"(data:image/(?:png|jpeg|jpg|svg\+xml);base64,[A-Za-z0-9+/=]+)"
